...
with raw_student_identities as (
  select
    ...
    jsonb_build_object('role_in_source', 'student', 'pilot_batch', 'stage_100') as source_payload,
    service_date,
    legacy_id
  from app_private.master_attendance_stage
  where source_payload->>'pilot_batch' = 'stage_100'
...
with raw_parent_identities as (
  select
    ...
    student_norm_phone as normalized_phone,
    jsonb_build_object('role_in_source', 'primary', 'pilot_batch', 'stage_100') as source_payload,
    service_date,
    legacy_id
  from app_private.master_attendance_stage
  where source_payload->>'pilot_batch' = 'stage_100'
...
with raw_instructor_identities as (
  select
    ...
    jsonb_build_object('role_in_source', 'instructor', 'pilot_batch', 'stage_100') as source_payload,
    service_date,
    legacy_id
  from app_private.master_attendance_stage
  where source_payload->>'pilot_batch' = 'stage_100'
...
where st.source_payload->>'pilot_batch' = 'stage_100'
...
begin
  ...
  where si.source_type = 'master_attendance_data'
    and si.source_payload->>'pilot_batch' = 'stage_100';
